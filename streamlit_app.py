...
def calculate_column_widths(headers, data, pdf, max_width=500, fixed_width=30):
    ...
    for j, header in enumerate(headers):  #CHANGED: Added header to check for "Details"
        ...
        width = pdf.get_string_width(str(header) or "") + 6  
        if header in ["PRODUCT DESC.", "SUPPLIER NAME"] :  
            width = max(width, 28) 
        else:
            width = min(width,20 )
...
def save_as_pdf(sheet, page_ranges, output_folder, file_prefix="XD"):
    ...
    for idx, cell_range in page_ranges.items():
        ...
        supp_idx = headers.index('SUPPLIER NAME') if 'SUPPLIER NAME' in headers else None
        if data.empty:
            print("Data is unexpectedly empty before file name creation.")
            print(cell_range)
            file_name = f"{idx}.pdf"
        else:
            file_name = (f"{file_prefix}_{sanitize_filename(data.iloc[0, supp_idx])} {sanitize_filename(data.iloc[0, month_idx])}.pdf"
                            if month_idx is not None and supp_idx is not None 
                            else f"{idx}.pdf"
                            )
        ...
        for j, header in enumerate(headers):
            ...
            ##if header in ['SUPPLIER NO.','BU','CATALOG','ITEM NUMBER','UNIT AMOUNT','RETRO AMOUNT','QUANTITY','CURRENCY', 'UOM','ORDER NO.','RECEIVED DATE']:
                #pdf.set_font('Arial', 'B', 7)
            #else:
                #pdf.set_font('Arial', 'B', 7)
        ...
        for row_idx, row in data.iterrows():
            ...
            for j, item in enumerate(row):
                col_letter = get_column_letter(j + 1)
                if headers[j] in ['PRODUCT DESC.','SUPPLIER NUMBER','SUPPLIER NAME']:
                    pdf.set_font('Arial', '', 6)
                else:
                    pdf.set_font('Arial', '', 7) 

                if isinstance(item, (datetime.datetime,pd.Timestamp)) and not pd.isna(item):
                ...
                if headers[j] in ['UNIT AMOUNT','RETRO AMOUNT'] and item is not None and not pd.isna(item):
                        try:
...
def main():
    ...
    st.title("Purchasing Retros Invoice Backups")
    uploaded_file = st.file_uploader("Upload your Excel file (Updated August 2025) ", type=["xlsx"])
    ...
    if uploaded_file is not None:
        ...
        file_prefix = uploaded_file.name[:2].upper()
        ...
        try:
            ...
            pdf_files = save_as_pdf(sheet, page_ranges, "generated_pdfs",file_prefix)
